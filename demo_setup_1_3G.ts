...
async function run() {
	const vm = await VAPI.VM.open({ ip: blade || "", protocol: "ws", towel });
	enforce(vm instanceof VAPI.AT1130.Root);
	console.log(`Setup demo on blade ${vm.raw.ip}`);
	await delete_all(vm);
	...
	await check_and_upload_bids(vm);
	await vm.close();
}
